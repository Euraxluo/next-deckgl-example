...
function ModeGroupButtons({modeGroup, mode, onSetMode}: any) {
    ...
    if (expanded) {
        subTools = (
            <SubTools>
                {modes.map((buttonConfig: { mode: any; content: any; title: string }, i: any) => (
                    <ModeButton
                        key={i}
...
export default function Toolbox({
    ...
    var saveData = React.useCallback(() => {
        ...
        if (JSON.stringify(history[history.length - 1]) === JSON.stringify(newData)) {
            return
        }
        // Only update history if data has changed
        if (JSON.stringify(newHistory[newHistory.length - 1]) !== JSON.stringify(newData)) {
            newHistory.push(newData);
        }
        history = newHistory;
        setHistoryIndex(newHistory.length - 1);
        localStorage.setItem('map-edit-toolbox-storage', JSON.stringify(newHistory));
                    ...
                    {showClearConfirmation && (
                        <SubTools>
                            <Button
                                ...
                                onClick={() => {
                                    saveData();
                                    geoJson.features = geoJson.features.filter((feature: any, index: number) => selectedFeatureIndexes.indexOf(index) === -1);
                                    onSetSelectedFeatureIndexes([]);
